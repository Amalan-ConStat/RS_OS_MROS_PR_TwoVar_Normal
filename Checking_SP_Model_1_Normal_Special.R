...
{
  Mean_X<-c(0,0)
  X<-rmvn(n = N, mu=Mean_X,Sigma=diag(1,2))
...
Theta<-as.matrix(cbind(1,0.5,seq(0.1,0.9,0.2)))
...
for (j in 1:nrow(Theta))
{
  ...
  for (i in 1:length(N))
    ...
    Plotting_Data %>%
      ...
      #geom_line(alpha=0.5)+
      xlab("X1")+ylab("log(lambda+1)")+theme_bw()+
    ...
    ggplot(.,aes(x=X2,y=Values,color=factor(Model)))+
      geom_point(alpha=0.5)+
      #geom_line(alpha=0.5)+
      xlab("X2")+ylab("log(lambda+1)")+theme_bw()+
      scale_color_viridis_d(guide = guide_legend(override.aes = list(alpha = 1) ) )+
      ggtitle("Comparing Real vs Assumed models",
              subtitle = paste("N =",Temp_N,
                               ", \U03F4_0 =",Theta[j,1],",\U03F4_1 =",Theta[j,2],",\U03F4_2 =",Theta[j,3]))+
...
Theta<-as.matrix(cbind(1,-0.3,0.3,seq(0.1,0.9,0.2)))
...
Theta<-as.matrix(cbind(1,0.3,0.3,-0.3,seq(0.1,0.9,0.2)))
...
for (j in 1:nrow(Theta))
{
  ...
  for (i in 1:length(N))
  {
    ...
    Plotting_Data %>%
      ...
      geom_point(alpha=0.5)+
      #geom_line(alpha=0.5)+
      xlab("X2")+ylab("log(lambda)")+theme_bw()+
      scale_color_viridis_d(guide = guide_legend(override.aes = list(alpha = 1) ) )+
      ggtitle("Comparing Real vs Assumed models",
              subtitle = paste("N =",Temp_N,
                               ", \U03F4_0 =",Theta[j,1],",\U03F4_1 =",Theta[j,2],
