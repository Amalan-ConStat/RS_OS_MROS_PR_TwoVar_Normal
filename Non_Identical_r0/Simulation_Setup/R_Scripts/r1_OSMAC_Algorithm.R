...
AlgTwoStp <- function(r1=r1, r2=r2,Y,X,Real_Data,n,Model) {
  if(Model=="Real_Model")
  {
    ...
    beta.mVc<-matrix(nrow = length(r1),ncol = ncol(X) )
    Utility_mVc<-matrix(nrow = length(r1),ncol = 3 )
    Bias_mVc<-matrix(nrow = length(r1),ncol = ncol(X) )
    
    beta.mMSE<-matrix(nrow = length(r1),ncol = ncol(X) )
    Utility_mMSE<-matrix(nrow = length(r1),ncol = 3 )
    Bias_mMSE<-matrix(nrow = length(r1),ncol = ncol(X) )
    ...
    for (i in 1:length(r1)) 
    {
      ...
      # mVc
      idx.mVc <- sample(1:n, r2-r1[i], T, PI.mVc)
      ...
      ## mMSE
      idx.mMSE <- sample(1:n, r2-r1[i], T, PI.mMSE)
    ...
    opt <- list("Est_Theta_mMSE"=cbind(r1,beta.mMSE),"Utility_mMSE"=Utility_mMSE,"Bias_mMSE"=cbind(r1,Bias_mMSE),
                #"Sample_mMSE"=Sample_mMSE,
                "Est_Theta_mVc"=cbind(r1,beta.mVc),"Utility_mVc"=Utility_mVc,"Bias_mVc"=cbind(r1,Bias_mVc)#,"Sample_mVc"=Sample_mVc
    )
    ...
    beta.mVc<-matrix(nrow = length(r1),ncol = ncol(x_Real) )
    Utility_mVc<-matrix(nrow = length(r1),ncol = 3 )
    Bias_mVc<-matrix(nrow = length(r1),ncol = ncol(x_Real) )
    
    beta.mMSE<-matrix(nrow = length(r1),ncol = ncol(x_Real) )
    Utility_mMSE<-matrix(nrow = length(r1),ncol = 3 )
    Bias_mMSE<-matrix(nrow = length(r1),ncol = ncol(x_Real) )
